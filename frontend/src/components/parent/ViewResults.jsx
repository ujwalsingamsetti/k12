...
export default function ParentViewResults() {
    ...
    useEffect(() => {
        ...
        const fetchSubmission = async () => {
            try {
                const res = await getParentSubmissionDetails(submissionId);
                const data = res.data;
                setSubmission(data);
                        ...
                        <div>
                            ...
                            <p className="text-sm font-medium opacity-90 leading-relaxed">
                                {submission.status === 'evaluating'
                                    ? 'The system is currently grading this submission.'
                                    : 'This submission is in the queue for processing.'}
                                {' '}This page will automatically refresh with the results.
                            </p>
                        </div>
                    </div>
                )}
                ...
                {submission.status === 'failed' && (
                    <div className="flex items-start gap-4 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/50 rounded-3xl px-6 py-6 mb-8 text-red-800 dark:text-red-400">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-medium opacity-90 leading-relaxed">
                                There was an error evaluating this answer sheet.
                            </p>
                        </div>
                    </div>
                )}
